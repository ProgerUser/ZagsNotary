...
public class BorderedTitledPane extends StackPane {
	...
	// todo other than TOP_LEFT other alignments aren't really supported correctly,
	// due to translation fudge for indentation of the title label in css => best to
	// implement layoutChildren and handle layout there.
	// todo work out how to make content the default node for fxml so you don't need
	// to write a <content></content> tag.
			...
			public void invalidated(Observable observable) {
				if (content.get() == null) {
					...
					if (!content.get().getStyleClass().contains("bordered-titled-content")) {
						content.get().getStyleClass().add("bordered-titled-content"); // todo would be nice to remove
																						// this style class when it is
																						// no longer required.
					}
