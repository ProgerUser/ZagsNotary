...
import ru.psv.mj.zags.doc.death.DEATH_CERT;
import ru.psv.mj.zags.doc.divorce.DIVORCE_CERT;
...
//		DeathList(MERCER_DIESHE);
	...
	void SaveCompare() {
		try {
			CallableStatement callStmt = conn
					.prepareCall("{ call Mercer.EditMercer(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }");
			callStmt.registerOutParameter(1, Types.VARCHAR);
			callStmt.setLong(2, mercer.getMERCER_ID());
			callStmt.setString(3, MERCER_OTHER.getText());
			if (!MERCER_DIEHE.getText().equals("")) {
				callStmt.setLong(4, Long.valueOf(MERCER_DIEHE.getText()));
			} else {
	...
	private void initialize() {
		try {
			...
			if (mercer.getMERCER_DSPMT_HE() != null && mercer.getMERCER_DSPMT_HE().equals("A")) {
				MERCER_DSPMT_HE.getSelectionModel().select("Свидетельство о расторжении брака");
			} else if (mercer.getMERCER_DSPMT_HE() != null && mercer.getMERCER_DSPMT_HE().equals("B")) {
				MERCER_DSPMT_HE.getSelectionModel().select("Свидетельство о смерти");
			}

			if (mercer.getMERCER_DSPMT_SHE() != null && mercer.getMERCER_DSPMT_SHE().equals("A")) {
				MERCER_DSPMT_SHE.getSelectionModel().select("Свидетельство о расторжении брака");
			} else if (mercer.getMERCER_DSPMT_SHE() != null && mercer.getMERCER_DSPMT_SHE().equals("B")) {
				MERCER_DSPMT_SHE.getSelectionModel().select("Свидетельство о смерти");
			}

			HeFio.setText(mercer.getHEFIO());
			if (mercer.getMERCER_HE() != 0) {
				MERCER_HE.setText(String.valueOf(mercer.getMERCER_HE()));
			}
			MERCER_HE_LNBEF.setText(mercer.getMERCER_HE_LNBEF());
			MERCER_HE_LNAFT.setText(mercer.getMERCER_HE_LNAFT());
			if (mercer.getMERCER_HEAGE() != 0) {
				MERCER_HEAGE.setText(String.valueOf(mercer.getMERCER_HEAGE()));
			}
			SheFio.setText(mercer.getSHEFIO());
			if (mercer.getMERCER_SHE() != 0) {
				MERCER_SHE.setText(String.valueOf(mercer.getMERCER_SHE()));
			}
			MERCER_SHE_LNBEF.setText(mercer.getMERCER_SHE_LNBEF());
			MERCER_SHE_LNBAFT.setText(mercer.getMERCER_SHE_LNBAFT());
			if (mercer.getMERCER_SHEAGE() != 0) {
				MERCER_SHEAGE.setText(String.valueOf(mercer.getMERCER_SHEAGE()));
			}
