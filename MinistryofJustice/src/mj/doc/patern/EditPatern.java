...
public class EditPatern {
	...
	void FindChildren(ActionEvent event) {
		...
		cus.Find_Cus(PС_CH, PС_CH_NAME, (Stage) PС_CH_NAME.getScene().getWindow());
	}
	...
	void AddChildren(ActionEvent event) {
		UtilCus cus = new UtilCus();
		cus.Add_Cus(PС_CH, PС_CH_NAME, (Stage) PС_M_NAME.getScene().getWindow(), conn);
	}
	...
	void AddMother(ActionEvent event) {
		UtilCus cus = new UtilCus();
		cus.Add_Cus(PС_M, PС_M_NAME, (Stage) PС_M_NAME.getScene().getWindow(), conn);
	}
	...
	void AddFather(ActionEvent event) {
		UtilCus cus = new UtilCus();
		cus.Add_Cus(PС_F, PС_F_NAME, (Stage) PС_M_NAME.getScene().getWindow(), conn);
	}
	...
	void FindMother(ActionEvent event) {
		...
		UtilCus cus = new UtilCus();
		cus.Find_Cus(PС_M, PС_M_NAME, (Stage) PС_M_NAME.getScene().getWindow());
		
//		CusList(PС_M, PС_M_NAME);
	}
	...
	void FindFather(ActionEvent event) {
		...
		UtilCus cus = new UtilCus();
		cus.Find_Cus(PС_F, PС_F_NAME, (Stage) PС_F_NAME.getScene().getWindow());
//		CusList(PС_F, PС_F_NAME);
	}
	...
	void Save(ActionEvent event) {
		try {
			...
			callStmt.setString(4, PС_SERIA.getText());
			callStmt.setDate(5, (PС_CRDATE.getValue() != null) ? java.sql.Date.valueOf(PС_CRDATE.getValue()) : null);
			callStmt.setString(6, PС_CRNAME.getText());
			callStmt.setDate(7, (PС_FZ.getValue() != null) ? java.sql.Date.valueOf(PС_FZ.getValue()) : null);
			callStmt.setDate(8, (PС_TRZ.getValue() != null) ? java.sql.Date.valueOf(PС_TRZ.getValue()) : null);
			callStmt.setString(9, PС_TYPE.getValue());
			if (!PС_M.getText().equals("")) {
				callStmt.setInt(10, Integer.valueOf(PС_M.getText()));
			} else {
				callStmt.setNull(10, java.sql.Types.INTEGER);
			}
			if (!PС_F.getText().equals("")) {
				callStmt.setInt(11, Integer.valueOf(PС_F.getText()));
			} else {
				callStmt.setNull(11, java.sql.Types.INTEGER);
			}
			if (!PС_CH.getText().equals("")) {
				callStmt.setInt(12, Integer.valueOf(PС_CH.getText()));
			} else {
				callStmt.setNull(12, java.sql.Types.INTEGER);
			}
			callStmt.setString(13, PС_AFT_MNAME.getText());
			callStmt.setString(14, PС_AFT_FNAME.getText());
			callStmt.setString(15, PС_AFT_LNAME.getText());
	...
	private void initialize() {
		try {
			...
			// сведения о ребенке
			PС_CH_NAME.setText(pat_cert.getCHILDFIO());
			if (pat_cert.getPС_CH() != 0) {
				PС_CH.setText(String.valueOf(pat_cert.getPС_CH()));
			}
			// сведения после установления отцовства
			PС_AFT_LNAME.setText(pat_cert.getPС_AFT_LNAME());
			PС_AFT_FNAME.setText(pat_cert.getPС_AFT_FNAME());
			PС_AFT_MNAME.setText(pat_cert.getPС_AFT_MNAME());
			// Акт о рождении
			if (pat_cert.getPC_ACT_ID() != 0) {
				PC_ACT_ID.setText(String.valueOf(pat_cert.getPC_ACT_ID()));
			}
			// Сведения о матери
			PС_M_NAME.setText(pat_cert.getMOTHERFIO());
			if (pat_cert.getPС_M() != 0) {
				PС_M.setText(String.valueOf(pat_cert.getPС_M()));
			}
			// Сведения об отце
			PС_F_NAME.setText(pat_cert.getFATHERFIO());
			if (pat_cert.getPС_F() != 0) {
				PС_F.setText(String.valueOf(pat_cert.getPС_F()));
			}
			// Тип основания для уст. отцовства
			...
			if (pat_cert.getPС_TYPE() != null) {
				if (pat_cert.getPС_TYPE().equals("A")) {
					PС_TYPE.getSelectionModel()
							.select("Совместное заявление родителей, не состоящихмежду собой в браке");
					PС_A.setVisible(true);

					PС_TRZ.setValue(pat_cert.getPС_TRZ());

				} else if (pat_cert.getPС_TYPE().equals("B")) {
					PС_TYPE.getSelectionModel().select("Заявление отца ребенка");
					PC_B.setVisible(true);

					PС_FZ.setValue(pat_cert.getPС_FZ());

				} else if (pat_cert.getPС_TYPE().equals("V1")) {

					PС_TYPE.getSelectionModel().select("Решение суда об установлении отцовства");
					PC_V.setVisible(true);

					PС_CRDATE.setValue(pat_cert.getPС_CRDATE());
					PС_CRNAME.setText(pat_cert.getPС_CRNAME());
				} else if (pat_cert.getPС_TYPE().equals("V2")) {
					PС_TYPE.getSelectionModel().select("Решение суда об установления факта признания");
					PC_V.setVisible(true);
